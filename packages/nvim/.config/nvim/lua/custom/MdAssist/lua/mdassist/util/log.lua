...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.MdAssist.config ~= nil and not _G.MdAssist.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[mdassist.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "mdassist.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h MdAssist.options` to read more."
        )
    end
end
