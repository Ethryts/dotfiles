...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.PyFix.config ~= nil and not _G.PyFix.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[pyfix.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "pyfix.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h PyFix.options` to read more."
        )
    end
end
